...
use json::JsonValue;
use websocket::{server::{InvalidConnection, NoTlsAcceptor, WsServer}, sync::{server::upgrade::Buffer, Client as WsClient, Reader, Writer}, Message, OwnedMessage, WebSocketError, WebSocketResult};
...
pub enum IncomingConnectionError {
    ...
    CouldNotAccept((TcpStream, io::Error)),
}
...
impl Display for IncomingConnectionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::CouldNotAccept((_, err)) => { "Could not accept: ".fmt(f)?; err.fmt(f) }
        }
    }
}
...
impl Server {
    ...
    pub fn accept_clients(self) -> impl Iterator<Item = Result<Client, IncomingConnectionError>> {
        self.0.into_iter().map(|connection| {
            ...
            let client = connection.accept().or_else(|e| Err(IncomingConnectionError::CouldNotAccept(e)))?;
            Ok(Client(client))
        })
    }
}
...
impl Client {
    pub fn split(self) -> Result<(MessageReader, MessageWriter), io::Error> {
        let (reader, writer) = self.0.split()?;
        Ok((MessageReader(reader), MessageWriter(writer)))
    }
}
...
pub struct MessageReader(Reader<TcpStream>);

pub struct MessageWriter(Writer<TcpStream>);
